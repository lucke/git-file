...
file.prototype.getFile = function() {
	...
	if (this.validFile())
	{
		...
		if (this.github) {
			...
			this.sendGet(this.url+"?github=1&repository="+repo+"&user="+user+"&op=5&blob="+this.file, this.displayFile, this.displayError, this.displayException);
		}
		else {
...
file.prototype.validFile = function() {
...
file.prototype.errorFile = function() {

	this.alert("Unsupported", "Sorry, unsupported file type.", EzWebExt.ALERT_ERROR);
...
file.prototype.setRepository = function(msg) {
	...
	if (data.length == 3)
	{
		file.repository = data[0]+";"+data[1];
		file.github = true;
	}
	else
	{
		file.repository = data[0];
		file.github = false;
	}
